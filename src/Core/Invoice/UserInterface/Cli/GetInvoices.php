...
class GetInvoices extends Command
{
    ...
    {
        $status = InvoiceStatus::tryFrom(strtolower($input->getArgument('status')));

        if (null === $status) {
            $output->writeln(
                sprintf(
                    '<error>Błędny status dozwolone wartości to %s</error>',
                    join(',', array_column(InvoiceStatus::cases(), 'value'))
                )
            );
